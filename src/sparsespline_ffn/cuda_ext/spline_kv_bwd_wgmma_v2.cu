...
spline_kv_bwd_wgmma_v2_kernel(
    ...
    // Per-chunk SMEM (re-used across CHUNKS_PER_PART)
    ...
    // ---- Main loop: iterate THIS partition's chunks ----
    #pragma unroll 1
    for (int chunk = 0; chunk < CHUNKS_PER_PART; chunk++) {
        ...
        // ---- Phase 2: cp.async load g_cores ----
        {
            ...
            for (int idx = tid; idx < total_rows; idx += blockDim.x) {
                ...
                const int n_global_idx = n_start + k_global;
                __nv_bfloat16* dst =
        ...
        cp_async_commit();
        cp_async_wait_all();
        // CRITICAL: cp_async_wait_all is per-thread.  We need a block-wide
        // sync so (a) every thread sees ALL g_cores loads from Phase 2
        // (Phase 3 reads g_cores[c] for c=0..R-1 — written by other threads),
        // and (b) Phase 1's W_cores zeros are guaranteed visible before any
        // thread starts writing B0/B1/B2 into W_cores in Phase 3.
        __syncthreads();
...
#define LAUNCH_BWD_WGMMA_V2(BN, BH, LP, RR, NPP) \
    do { \
        ...
        spline_kv_bwd_wgmma_v2_kernel<BN, BH, LP, RR, NPP> \
            <<<grid, block, 0, stream>>>( \
...
//   dz : [N, H]    fp32
// =============================================================================
std::vector<torch::Tensor> spline_kv_bwd_wgmma_v2_cuda(
    ...
    if (R == 32 && L == 22) {
        LAUNCH_BWD_WGMMA_V2(64, 8, 24, 32, 256);
    } else if (R == 32 && L == 16) {
        LAUNCH_BWD_WGMMA_V2(64, 8, 16, 32, 256);
    } else if (R == 32 && L == 32) {
        LAUNCH_BWD_WGMMA_V2(64, 8, 32, 32, 256);
    } else if (R == 64 && L == 22) {
        LAUNCH_BWD_WGMMA_V2(64, 8, 24, 64, 256);
    } else if (R == 64 && L == 16) {
        LAUNCH_BWD_WGMMA_V2(64, 8, 16, 64, 256);
    } else if (R == 64 && L == 32) {
        LAUNCH_BWD_WGMMA_V2(64, 8, 32, 64, 256);
    } else {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("spline_kv_bwd_wgmma_v2_cuda", &spline_kv_bwd_wgmma_v2_cuda,
