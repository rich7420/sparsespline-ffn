...
spline_kv_bwd_wgmma_v4_kernel(
    const __nv_bfloat16* __restrict__ z,
    const __nv_bfloat16* __restrict__ C,
    const __nv_bfloat16* __restrict__ g_delta,
    float* __restrict__ dC_scratch,    // [N_TILE, H, L, R] fp32 — UNIQUE per-block writes
    float* __restrict__ dz,
    const int N, const int H, const int L,
    const int N_TILE,                   // grid_x dim, used for stride into dC_scratch
        ...
        for (int n_tile = 0; n_tile < N_TILES; n_tile++) {
            ...
            for (int chunk = 0; chunk < 4; chunk++) {
                ...
                for (int e = 0; e < 4; e++) {
                    ...
                    if (j_global < H && b < L) {
                        // v4: write to scratch[n_tile, j_global, b, c] (UNIQUE
                        // per (n_tile, h_block) — no atomic needed since each
                        // CTA owns its scratch slice.
                        // Layout: scratch[N_TILE, H, L, R] fp32.
                        const long scratch_idx =
                            ((long)pid_n * H + j_global) * L * R + b * R + c;
                        dC_scratch[scratch_idx] = acc[frag_idx];
                    }
                }
            }
        }
...
// v4 reduce kernel: dC[h, b, c] = bf16(Σ_n_tile dC_scratch[n_tile, h, b, c])
__global__ void v4_reduce_dC_kernel(
    const float* __restrict__ dC_scratch,    // [N_TILE, H, L, R] fp32
    __nv_bfloat16* __restrict__ dC_out,       // [H, L, R] bf16
    const int H, const int L, const int R, const int N_TILE
) {
    ...
    for (int p = 0; p < N_TILE; p++) {
        sum += dC_scratch[((long)p * H + h) * L * R + b * R + c];
    }
    dC_out[h * L * R + b * R + c] = f2bf(sum);
}

#define LAUNCH_BWD_WGMMA_V4(BN, BH, LP, RR) \
    do { \
        ...
        spline_kv_bwd_wgmma_v4_kernel<BN, BH, LP, RR><<<grid, block, 0, stream>>>( \
        ...
        const int reduce_total = H * L * RR; \
        const int reduce_block = 128; \
        const int reduce_grid  = (reduce_total + reduce_block - 1) / reduce_block; \
        v4_reduce_dC_kernel<<<reduce_grid, reduce_block, 0, stream>>>( \
            dC_scratch_ptr, (__nv_bfloat16*)dC_bf16_ptr, H, L, RR, blocks_n); \
    } while(0)

}  // namespace

// v4: returns (dC bf16, dz fp32).
// Internally allocates fp32 scratch [N_TILE, H, L, R] for atomic-free dC
// accumulation, then runs reduce kernel.
std::vector<torch::Tensor> spline_kv_bwd_wgmma_v4_cuda(
    ...
    const int N_TILE_count = (N + BN - 1) / BN;
    ...
    // dC bf16 output, dz fp32 output, scratch fp32 internal.
    torch::Tensor dC_out = torch::empty({H, L, R}, bf16_opts);
    torch::Tensor dz_out = torch::empty({N, H}, fp32_opts);
    torch::Tensor dC_scratch = torch::empty({N_TILE_count, H, L, R}, fp32_opts);

    void* z_ptr       = z.data_ptr();
    void* C_ptr       = C.data_ptr();
    void* g_delta_ptr = g_delta.data_ptr();
    void* dC_bf16_ptr = dC_out.data_ptr();
    float* dC_scratch_ptr = dC_scratch.data_ptr<float>();
    float* dz_ptr     = dz_out.data_ptr<float>();

    if (R == 32 && L == 22) {
        LAUNCH_BWD_WGMMA_V4(128, 8, 24, 32);
    } else if (R == 32 && L == 16) {
        LAUNCH_BWD_WGMMA_V4(128, 8, 16, 32);
    } else if (R == 32 && L == 32) {
        LAUNCH_BWD_WGMMA_V4(128, 8, 32, 32);
    } else if (R == 64 && L == 22) {
        LAUNCH_BWD_WGMMA_V4(128, 8, 24, 64);
    } else if (R == 64 && L == 16) {
        LAUNCH_BWD_WGMMA_V4(128, 8, 16, 64);
    } else if (R == 64 && L == 32) {
        LAUNCH_BWD_WGMMA_V4(128, 8, 32, 64);
    } else {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("spline_kv_bwd_wgmma_v4_cuda", &spline_kv_bwd_wgmma_v4_cuda,
          "v4 bwd: scratch + reduce, no global atomic on dC. Returns (dC bf16, dz fp32).",
