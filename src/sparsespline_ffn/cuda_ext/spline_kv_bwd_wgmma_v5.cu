// Backward v5 — register-resident dC + fp16 wgmma (precision-safe).
//
// Goals:
...
spline_kv_bwd_wgmma_v5_kernel(
    const __nv_bfloat16* __restrict__ z,
    const __half* __restrict__ C_fp16,         // [H, L, R] fp16 (caller cast)
    ...
    // ---- Phase A: load C[h_start:h_start+BH] ONCE into SMEM (XOR-swizzled) ----
    // (C is the same across all chunks of this block.)
    // c_start ∈ {0, 8, 16, 24} maps to swz_c(j, c_start) ∈ same set after
    // the XOR-by-((j&3)<<3) — a permutation of the 4 aligned 8-fp16 blocks.
    {
        ...
        for (int idx = tid * 8; idx < total; idx += blockDim.x * 8) {
            ...
            cp_async_16_h(&C_smem[j_local][b][swz_c(j_local, c_start)],
                          valid ? &C_fp16[j_global * L * R + b * R + c_start] : nullptr,
                          valid);
        }
    }
    cp_async_commit();    // group 0 = C load
    ...
    // Drain Phase A (group 0) and chunk 0 (group 1); leave chunk 1 (group 2)
    // in flight so iter 0 starts with one prefetch already in motion.
    if (chunks_per_block > 1) {
    ...
    for (int chunk = 0; chunk < chunks_per_block; chunk++) {
        ...
        // Does NOT touch W_cores or acc[] → safe alongside wgmma.
        {
            ...
            for (int p = tid; p < total_pairs; p += blockDim.x) {
                ...
                #pragma unroll
                for (int c = 0; c < R; c++) {
                    const int c_swz = swz_c(j_local, c);
                    const float g  = h2f(g_cores[buf][G_OFF(n_local, c)]);
                    const float c0 = h2f(C_smem[j_local][bin_idx + 0][c_swz]);
                    const float c1 = h2f(C_smem[j_local][bin_idx + 1][c_swz]);
                    const float c2 = h2f(C_smem[j_local][bin_idx + 2][c_swz]);
...
__global__ void spline_kv_bwd_v5_reduce_kernel(
...
#define LAUNCH_BWD_V5(BN, BH, LP, RR, NPARTS) \
    do { \
        const int chunks_per_block = (N / NPARTS) / BN; \
        TORCH_CHECK(chunks_per_block > 0,    "v5 needs N/NPARTS/BN > 0"); \
        TORCH_CHECK((N / NPARTS) % BN == 0,  "v5 needs N/NPARTS divisible by BN"); \
        const int blocks_h = (H + (BH) - 1) / (BH); \
        dim3 grid(NPARTS, blocks_h, 1); \
        dim3 block(128, 1, 1); \
        auto stream = c10::cuda::getCurrentCUDAStream(); \
        spline_kv_bwd_wgmma_v5_kernel<BN, BH, LP, RR, NPARTS> \
            <<<grid, block, 0, stream>>>( \
        ...
        spline_kv_bwd_v5_reduce_kernel<<<reduce_grid, 256, 0, stream>>>( \
...
// =============================================================================
std::vector<torch::Tensor> spline_kv_bwd_wgmma_v5_cuda(
    ...
    if (R == 32 && L == 22) {
        LAUNCH_BWD_V5(128, 8, 24, 32, N_PARTS_DEFAULT);
    } else if (R == 32 && L == 16) {
        LAUNCH_BWD_V5(128, 8, 16, 32, N_PARTS_DEFAULT);
    } else if (R == 32 && L == 32) {
        LAUNCH_BWD_V5(128, 8, 32, 32, N_PARTS_DEFAULT);
    } else if (R == 64 && L == 22) {
        LAUNCH_BWD_V5(128, 8, 24, 64, N_PARTS_DEFAULT);
    } else {
        TORCH_CHECK(false, "spline_kv_bwd_wgmma_v5: unsupported (R, L)");
    }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("spline_kv_bwd_wgmma_v5_cuda", &spline_kv_bwd_wgmma_v5_cuda,
          "v5 bwd: register-resident dC + fp16 wgmma. Returns (dC bf16, dz fp32).",
